...
public sealed class Repository<T> : IRepository<T> where T : IEntity<T>
{
    ...
    public async Task<T> Update(T entity, CancellationToken cancellationToken = default)
    {
        ...
        if (existingEntity is null)
            throw new EntityDoesNotExistException(
                $"Update failed. Entity of type {typeof(T).Name} with id: {entity.Id} does not exist.");

        var sql
            = $"UPDATE {_tableName} SET {string.Join(", ", columnNames.Zip(parameterNames, (column, parameter) => $"{column} = {parameter}"))} WHERE Id = @Id";
    ...
    public async Task<T> Delete(int id, CancellationToken cancellationToken = default)
    {
        ...
        if (entity is null)
            throw new EntityDoesNotExistException(
                $"Delete failed. Entity of type {typeof(T).Name} with id: {id} does not exist.");
